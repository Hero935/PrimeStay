...
### 6.1 訂閱主體原則
系統採用「**工作區中心制 (Workspace-Centric)**」：
1. **方案與組織綁定**：`SubscriptionPlan` 存放於 `Organization` 模型。
2. **額度繼承**：所有在該組織下的 `Property` 與 `Member` 共享該組織的方案限制。
3. **身分切換**：當用戶切換工作區時，前端需重新加載該組織的方案狀態。
