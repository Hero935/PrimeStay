...
### 2.1 零滾動佈局規範 (Zero-Scroll Design Principles)
核心治理介面須鎖定屏高至 100vh，嚴禁產生全域滾動條，確保所有戰略指標 (Red Lines) 始終位於第一屏。
...
### 4.2 全域資產中樞 (The Nexus Pulse - /admin/management)
*   **定位**：全域架構導航儀 (Architect Explorer)。用於觀察跨組織、地理區域的資產血緣關係，並進行即時損害評估。
*   **核心視覺規範 (Nexus Pulse)**：
    - **脈動系統**：節點具備動態 CSS 脈動點 (綠: 健康、紅: 停權、藍: 閒置)。
    - **血緣麵包屑**：顯示「房東 > 房源 > 租客」的 DNA 血緣。
    - **極致扁平化 (Zero-Scroll)**：利用 100vh 視區鎖定，將診斷 DNA 圖表與資產清單垂直並行。
